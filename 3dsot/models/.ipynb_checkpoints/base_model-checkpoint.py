...
class BaseModel(pl.LightningModule):
    ...
    def evaluate_one_sequence(self, sequence):
        ...
        distances = []
        results_bbs = []
        save_dict = copy.deepcopy(sequence)
        for frame_id in range(len(sequence)):  # tracklet
            ...
            save_dict[frame_id]["results_bb"] = results_bbs[-1]
            del save_dict[frame_id]['pc']
        ## used for future vizualization of results
        # dbfile = open(f"probs-anno", 'wb')
...
class MatchingBaseModel(BaseModel):
    ...
    def compute_loss(self, data, output):
        ...
        loss_vote = F.smooth_l1_loss(vote_xyz, box_label[:, :, :3].expand_as(vote_xyz), reduction='none')  # B,N,3
        loss_vote = (loss_vote.mean(2) * seg_label).sum() / (seg_label.sum() + 1e-06)

        dist = torch.sum((proposal_center - box_label[:, :64, :3]) ** 2, dim=-1)
        ...
        loss_box = F.smooth_l1_loss(estimation_boxes[:, :, :4],
                                    box_label[:, :64, :4].expand_as(estimation_boxes[:, :, :4]),
                                    reduction='none')
