...
def parse_config():
    ...
    parser.add_argument('--reference_BB', type=str, default="previous_result", help='search area sampling')
    parser.add_argument('--shape_aggregation', type=str, default="firstandprevious", help='template sampling')
    parser.add_argument('--category_name', type=str, default="Car", help='category')
...
if not cfg.test:
    ...
    ## sort and print all checkpoints
    print_all = False
    if print_all:
        ...
        ckpt_path = './train_ctraormer/lightning_logs/version_0/checkpoints/'
        ckpts = [f for f in listdir(ckpt_path) if isfile(join(ckpt_path, f))]
        ckpts.remove('last.ckpt')
        ckpts.sort(key=sortbybatch)
        print(ckpts)
        for ckpt in ckpts:
            print("'"+i+"'", end=" ")
        #     trainer = pl.Trainer(gpus=-1, accelerator='ddp', default_root_dir=cfg.log_dir,
